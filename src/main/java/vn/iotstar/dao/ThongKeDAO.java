...
public class ThongKeDAO extends DBConnection {
	public int[] getThongKeTheoThang(int year, int id) {
		...
		String sql = "select sum(cost_Sum), MONTH(created_At) from [Order] where storeid=? and YEAR(created_At)=? group by MONTH(created_At) order by MONTH(created_At)";
		try {
	...
	public int[] getThongKeTheoThang(int year) {
		...
		String sql = "select sum(cost_Sum), MONTH(updated_At) from [Order] where YEAR(updated_At)=? group by MONTH(updated_At) order by MONTH(updated_At)";
		try {
			Connection conn = super.getConnection();
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setInt(1, year);
	...
	public List<StoreModels> getTop8Store(int year){
		...
		String sqlString="with b1\r\n"
				+ "as\r\n"
				+ "(\r\n"
				+ "	select sum(cost_sum) as sumcost, storeid\r\n"
				+ "	from [order]\r\n"
				+ "	where Year(updated_At)=year(getdate())\r\n"
				+ "	group by storeid\r\n"
				+ "\r\n"
				+ ")\r\n"
	...
	public int countOrderOfMonth() {
		String sql="select count(*) from [order] where month(updated_At)=month(getdate()) and YEAR(updated_At)=YEAR(getDAte())";
		try
		{
			Connection conn= new DBConnection().getConnection();
			PreparedStatement ps= conn.prepareStatement(sql);
	...
	public int sumOfMonth() {
		String sql="select sum(cost_sum) from [order] where Month(updatedAt)=month(getdate()) and YEAR(updatedAt)=YEAR(getDAte())";
		try
		{
			Connection conn= new DBConnection().getConnection();
			PreparedStatement ps= conn.prepareStatement(sql);
