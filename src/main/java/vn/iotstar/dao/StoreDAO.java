...
public class StoreDAO extends DBConnection {
	...
	{
		String sql="Select storeid, name, avatar, is_Open from Stores where storeID=?";
		try {
...
public void editStore(StoreModels store, int id) {
	...
	String sql="UPDATE Stores SET name=?,avatar=?, is_Open=?, updated_At=? WHERE storeID=?";
	try {
		Connection conn= new DBConnection().getConnection();
		PreparedStatement ps=conn.prepareStatement(sql);
	...
	public List<StoreModels> getAll(){
		...
		String sql="select Stores.storeID,  Stores.name, Stores.ownerID, Stores.is_Active, Stores.is_Open, Stores.avatar, Stores.point, Stores.rating, Stores.created_At, Stores.updated_At, Users.name from Stores inner join Users on Stores.ownerID=Users.userID";
		try {
	...
	public List<StoreModels> findAllPage(int indexp){
		List <StoreModels> stores= new ArrayList<StoreModels>();
		String sql="select Stores.storeID,  Stores.name, Stores.ownerID, Stores.is_Active, Stores.is_Open, Stores.avatar, Stores.point, Stores.rating, Stores.created_At, Stores.updated_At, Users.name from Stores inner join Users on Stores.ownerID=Users.userID ORDER BY storeID DESC OFFSET ?*5 rows fetch next 5 rows only";
		try {
			conn = new DBConnection().getConnection();
			ps = conn.prepareStatement(sql);
			ps.setInt(1, indexp);
	...
	public StoreModels get(int id) {
		String sql="select Stores.storeID,  Stores.name, Stores.ownerID, Stores.is_Active, Stores.is_Open, Stores.avatar, Stores.point, Stores.rating, Stores.created_At, Stores.updated_At, Users.name from Stores inner join Users on Stores.ownerID=Users.userID where storeID=?";
		try {
	...
	public void edit(int id) {
		String sql="UPDATE Stores SET is_Active=? WHERE StoreID=?";
		try {
			Connection conn= super.getConnection();
			PreparedStatement ps=conn.prepareStatement(sql);
	...
	public void insert(String name,int id) {
		String sql="insert into Stores(name,ownerID,is_Active,is_Open,point,rating,created_At,updated_At) values (?,?,?,?,?,?,?,?)";
		try {
			Connection conn= super.getConnection();
			PreparedStatement ps= conn.prepareStatement(sql);
	...
	{
		String sql="UPDATE Stores SET is_Active='false' where StoreID=?";
