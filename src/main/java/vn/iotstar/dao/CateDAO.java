...
public class CateDAO extends DBConnection{
	...
	public void insert(CategoryModels category) {
		String sql="INSERT INTO Category(name,slug,image,is_Deleted,created_At,updated_At) VALUES (?,?,?,?,?,?)";
		try {
			Connection conn=super.getConnection();
			PreparedStatement ps= conn.prepareStatement(sql);
	...
	{
		String sql="UPDATE Category SET name=?, image=?, is_Deleted=?, updated_At=? WHERE categoryID=?";
		try {
			Connection conn = super.getConnection();
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setString(1, category.getNamecate());
	...
	public List<CategoryModels> getCateOfStore(int id) {
		List<CategoryModels> categories = new ArrayList<CategoryModels>();
		String sql = "SELECT distinct(Category.categoryID), Category.name, Category.image, Category.is_Deleted,Category.created_At,Category.updated_At FROM Category inner join Products on Category.categoryID=Products.categoryID where Products.storeID=?";
		try {
			Connection conn = super.getConnection();
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setInt(1,id);
