...
public int countTT(int tt) {
	...
	{
		String sql="insert into [dbo].[Order](userID,address,phone,status,statusTT,cost_Sum,StoreID,created_At,updated_At) values(?,?,?,N'Chưa xác nhận',N'Chưa thanh toán',?,?,?,?)";
		try {
			conn=new DBConnection().getConnection();
			ps=conn.prepareStatement(sql);
			ps.setInt(1, userid);
	...
	public List<DonHangVM> getGH_ST(int id,int cate) {
		...
		String sql = "with b1\r\n"
				+ "as\r\n"
				+ "(select [Order].orderID, Users.name,Stores.name as nameStore, [Order].address,[Order].storeID, [Order].phone, [Order].statusTT, [Order].status, [Order].cost_Sum, [Order].created_At, [Order].updated_At, [Order].shipperID \r\n"
				+ "from (Users left join [Order]\r\n"
				+ "on Users.userID=[Order].userID ) inner join Stores on [Order].storeID=Stores.storeID\r\n"
				+ ")\r\n"
				+ "select b1.orderID, b1.name, b1.nameStore, b1.address, b1.phone, b1.statusTT, b1.status, b1.cost_Sum,\r\n"
				+ "b1.created_At, b1.updated_At, Users.name as nameShipper\r\n"
	...
	{
		String sqlString="select sum(cost_Sum) from [order] where MONTH(created_At)=? and storeID=?";
		try {
			Connection conn= new DBConnection().getConnection();
			PreparedStatement ps= conn.prepareStatement(sqlString);
			ps.setInt(1, thang);
	...
	{	
		...
		String sql="with b1\r\n"
				+ "as\r\n"
				+ "(select [Order].orderID, Users.name,Stores.name as nameStore, [Order].address, [Order].phone, [Order].statusTT, [Order].status, [Order].cost_Sum, [Order].created_At, [Order].updated_At, [Order].shipperID \r\n"
				+ "from (Users left join [Order]\r\n"
				+ "on Users.userID=[Order].userID ) inner join Stores on [Order].storeID=Stores.storeID\r\n"
				+ ")\r\n"
				+ "select b1.orderID, b1.name, b1.nameStore, b1.address, b1.phone, b1.statusTT, b1.status, b1.cost_Sum,\r\n"
	...
	public DonHangModels getOrder(int orderid) {
		...
		String sql="with b1\r\n"
				+ "as\r\n"
				+ "(select [Order].orderID,[Order].userID,[Order].storeID, Users.name,Stores.name as nameStore, [Order].address, [Order].phone, [Order].statusTT, [Order].status, [Order].cost_Sum, [Order].created_At, [Order].updated_At, [Order].shipperID \r\n"
				+ "from (Users inner join [Order]\r\n"
				+ "on Users.userID=[Order].userID ) inner join Stores on [Order].storeID=Stores.storeID\r\n"
				+ ")\r\n"
				+ "select b1.orderID,b1.userid, b1.name,b1.storeid, b1.nameStore, b1.address, b1.phone, b1.statusTT, b1.status, b1.cost_Sum,\r\n"
				+ "b1.created_At, b1.updated_At,b1.shipperID, Users.name as nameShipper\r\n"
				+ "from b1 inner join Users\r\n"
				+ "on b1.shipperID=Users.userID\r\n"
				+ "where b1.orderID=?";
		try {
