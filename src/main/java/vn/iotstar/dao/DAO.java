...
public class DAO {
	...
	public List<ProductsModels> top6store(int storeID) {
		List<ProductsModels> list = new ArrayList<>();
		String sql = "Select top 6 * from Products where storeID=? order by sold DESC ";
	...
	public List<ProductsModels> getpage(int id,int cate) {
		List<ProductsModels> list = new ArrayList<>();
		String sql = "SELECT * FROM Products where categoryID=? and is_Selling='True' and is_Active='True'  ORDER BY productid OFFSET ? ROWS FETCH NEXT 10 ROWS ONLY";
	...
	public int countsp(int cateid) {
		String sql = "select count(*) from Products where categoryID=? and is_Selling='True'";
		try {
			conn = new DBConnection().getConnection();
			ps = conn.prepareStatement(sql);
			ps.setInt(1, cateid);
	...
	public ProductsModels getIdSanpham(int id) 
	{
		...
		while(rs.next())
		{
			return new  ProductsModels(rs.getInt(1), rs.getString(2), rs.getString(4), rs.getInt(5), rs.getInt(7),
					rs.getInt(8), rs.getInt(11), rs.getInt(12), rs.getFloat(13),rs.getString(16));
		}
		}
		catch(Exception e)
		{
