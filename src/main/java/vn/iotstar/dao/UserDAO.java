	...
	public UserModels getAdmin(int id) {
		...
		try {
			...
			while (rs.next()) {
				...
				user.setHash_password(rs.getString(6));
...
public UserModels admin_login(String email, String pass) {
	...
	{	
		...
		while(rs.next())
		{
			return new UserModels(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(6),rs.getString(8),rs.getInt(9),rs.getInt(7));
		}
		}
		catch(Exception e)
		{
...
/////Thêm mới nè
	...
	public int countListFollow(int id) {
		String sql="select  count(*) from Users, UserFollowStore,Stores  where Users.userID=UserFollowStore.userID and Stores.storeID=UserFollowStore.storeID and Users.userID=? and UserFollowStore.is_Deleted='false'";
		try {
			Connection conn=new DBConnection().getConnection();
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setInt(1, id);
	...
	public void unfollow(int userid, int storeid) {
		String sql=" UPDATE UserFollowStore SET is_Deleted='true' where userid=? and storeid=?";
		try {
			Connection conn=new DBConnection().getConnection();
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setInt(1, userid);
